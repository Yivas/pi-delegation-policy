...
An optional Advisor does not require delegation and does not change the ordinary-role minimum. When it is configured and valid, the explicit `advisor_ask` tool can ask it one bounded question during a run, and the advisor executes no work of its own. With no advisor configured, that tool only answers `advisor-unavailable` and the agent continues. Read [limits and privacy](/pi-delegation-policy/limits-and-privacy/#advisor-limits) for the input, the bounded window, the six error codes, and retention.
