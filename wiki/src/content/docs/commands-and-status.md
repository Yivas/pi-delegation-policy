...
Use `/delegate` in Pi's TUI to open the keyboard-first editor. `Alt+G` opens the same editor when available. The editor requires TUI mode; these command arguments remain available in other modes. The published `0.7.0` package supports `off`, `normal`, `aggressive`, and `orchestrator`, and retains its permissive `orchestrator` policy; the stricter `D:ORCH` action below is Unreleased:
...
| `D:ORCH` | A valid `orchestrator` configuration is active.              | Unreleased: delegate all transferable execution first; retain final acceptance. Published `0.7.0` retains its permissive policy. |
