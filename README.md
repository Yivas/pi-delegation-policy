...
> **Status:** Version **0.14.0** is the latest published package and supports `off`, `normal`, `aggressive`, and `orchestrator`. The package peer range is Pi `>=0.84.3`; Pi `0.84.3` is the minimum supported version and Pi `0.85.1` is the explicitly checked baseline.
>
