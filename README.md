...
> **Status:** Version **0.13.0** is the latest published package and supports `off`, `normal`, `aggressive`, and `orchestrator`. The package peer range is Pi `>=0.84.3`; Pi `0.84.3` is the minimum supported version and Pi `0.85.1` is the explicitly checked baseline.
>
...
When the advisor is configured and valid, the injected policy tells the main agent to launch `pi-delegation-policy.advisor`, the profile that ships in this package, as a normal subagent through Pi's own subagent mechanism, with the exact configured model and that role's thinking policy. Consult it when a decision is ambiguous, when undoing it would be costly, when a risk remains that the main agent cannot resolve alone, or when a substantial doubt about architecture, plan, tooling, or approach could change the outcome. When a decision belongs to the user, the main agent consults the advisor first and brings options, trade-offs, and a recommendation together with the question. These are triggers, not a quota: routine decisions need no call, and nothing enforces one.
