...
> **Status:** Version **0.15.0** supports `off`, `normal`, `aggressive`, and `orchestrator` and requires Pi `0.87.1` or later (`@earendil-works/pi-coding-agent >=0.87.1`). Version `0.14.1` supports Pi `0.84.3` or later (`@earendil-works/pi-coding-agent >=0.84.3`). Pi `0.87.1` is the verified baseline for per-request policy updates.
>
...
- Validate active configurations before injecting one policy block. Version `0.15.0` refreshes it before each Pi `0.87.1+` LLM request, including subsequent requests in the same turn; version `0.14.1` applies changes on the next agent run.
...
5. With version `0.15.0` on Pi `0.87.1+`, applied changes affect the **next LLM request**, including another request in the current turn. Version `0.14.1` applies changes on the next agent run. A request already in progress and subagents already launched keep their existing state.
