...
> **Status:** Version **0.12.0** is the latest published package and supports `off`, `normal`, `aggressive`, and `orchestrator`. The package peer range is Pi `>=0.84.3`; Pi `0.84.3` is the minimum supported version and Pi `0.85.1` is the explicitly checked baseline.
>
...
- Optionally add an Advisor model and consult it on demand with the explicit `advisor_ask` tool; it advises and executes no work.
...
**Advisor** is an optional second consultation model, off by default and not part of the ordinary-role minimum. Configure it as an exact reference in global defaults, or `null` in a session to turn it off, with its own optional thinking policy. When it is configured and valid, the main agent may call the explicit `advisor_ask` tool with a question, optional extra context, and the thinking level for that call. The advisor answers in bounded plain text — at most 8 KiB, returned with the model and level that produced it — and executes no work of its own. Configuring nothing changes nothing: `advisor_ask` then answers `advisor-unavailable` and the agent continues. A configured advisor whose model is missing, out of scope, or unauthenticated produces `D:ERR` and injects no policy; because that check is shared, it also leaves the ContextShunt reader unauthorized. Using the tool sends the advisor's model a bounded window of the current conversation; the [limits and privacy reference](https://yivas.github.io/pi-delegation-policy/limits-and-privacy/) states exactly what that window includes, what it excludes, the size cap, and what the executor and its provider may retain.
