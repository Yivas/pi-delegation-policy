...
- Optionally add an Advisor role and launch it as a subagent with the profile that ships in this package; it advises and executes no work.
...
**Advisor** is an optional consultation role, off by default and not part of the ordinary-role minimum. Configure it as an exact reference in global defaults, or `null` in a session to turn it off, with its own optional thinking policy. When it is configured and valid, the injected policy tells the main agent to launch `pi-delegation-policy.advisor`, the profile that ships in this package, as a normal subagent through Pi's own subagent mechanism, using the exact configured model and that role's thinking policy. The profile advises and executes no work: it has no filesystem, network, conversation, or tool access beyond the task text it receives and the plain-text reply it returns. Consult it when a decision is ambiguous, when undoing it would be costly, or when a risk remains that the main agent cannot resolve alone; those are triggers, not a quota, and nothing forces a call. Configuring nothing changes nothing: no advisor section is injected and no launch is required. What the advisor receives is what the main agent writes into the task, and it travels through the same host-authorized external executor and provider with their normal retention, as the [limits and privacy reference](https://yivas.github.io/pi-delegation-policy/limits-and-privacy/) states. A configured advisor whose model is missing, out of scope, or unauthenticated produces `D:ERR` and injects no policy; because that check is shared, it also leaves the ContextShunt reader unauthorized.
