...
- State the advisor's current evidence ceiling: its end-to-end path through the external executor is not verified yet. It waits for the same executor re-verification as the ContextShunt reader and fails closed with `advisor-unavailable` until then, and the launch contract is covered only against a simulated preflight, never a real executor response.
- Move the pinned external executor protocol version to `0.70.0`. The binding is unchanged: another executor build still fails closed.
